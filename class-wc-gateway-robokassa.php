...
/*
  ...
  Author URI: https://github.com/denysrodman
 */
...
function woocommerce_init()
{
    ...
    class WC_Gateway_Robokassa extends WC_Payment_Gateway
    {
        ...
        <?php else : ?>
            <div class="inline error"><p><strong><?php _e('Шлюз отключен', 'woocommerce'); ?></strong>: <?php _e(
                        'ROBOKASSA не поддерживает валюты Вашего магазина.',
                        'woocommerce'
                    ); ?></p></div>
        <?php
        endif;
        ...
        function init_form_fields()
        {
            $this->form_fields = array(
                ...
                'description' => array(
                    ...
                    'type' => 'textarea',
                    'description' => __(
                        'Описанием метода оплаты которое клиент будет видеть на вашем сайте.',
                        'woocommerce'
                    ),
                ...
                'instructions' => array(
                    ...
                    'type' => 'textarea',
                    'description' => __(
                        'Инструкции, которые будут добавлены на страницу благодарностей.',
                        'woocommerce'
                    ),
                    'default' => 'Оплата с помощью robokassa.'
                )
        ...
        public function generate_form($order_id)
        {
            ...
            foreach ($args as $key => $value) {
                $args_array[] = '<input type="hidden" name="' . esc_attr($key) . '" value="' . esc_attr(
                        $value
                    ) . '" />';
            }
            ...
            return
                ...
                implode("\n", $args_array) .
                '<input type="submit" class="button alt" id="submit_robokassa_payment_form" value="' . __(
                    'Оплатить',
                    'woocommerce'
                ) . '" /> ' . "\n" .
                '</form>';
        ...
        function check_ipn_request_is_valid($posted)
        {
            ...
            if (strtoupper($posted['SignatureValue']) ==  strtoupper(md5("$out_summ:$inv_id:$this->robokassa_key1"))) {
                return true;
            } else if(strtoupper($posted['SignatureValue']) ==  strtoupper(md5("$out_summ:$inv_id:$this->robokassa_key2"))){
                return true;
            }
